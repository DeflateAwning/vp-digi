...
void TIM1_UP_IRQHandler(void)
{
	...
	if(afskCfg.usePWM)
	{
		TIM6->CCR1 = modState.dacSine[modState.dacSineIdx];
	}
	else
	{
...
void Afsk_txTestStart(TxTestMode type)
{
	...
	NVIC_DisableIRQ(DMA1_Channel1_IRQn); //disable RX DMA interrupt
	NVIC_EnableIRQ(TIM1_CC_IRQn); //enable timer 1 for PWM // TODO: confirm that TIM1_CC_IRQn is a fine replacement for TIM1_UP_IRQn
	...
	else //alternating tones
	{
		...
		TIM7->PSC = 71; //72/72=1 MHz
		TIM7->DIER = TIM_DIER_UIE; //enable interrupt
		TIM7->ARR = modState.baudRate; //set timer interval
...
void Afsk_txTestStop(void)
{
	...
	TIM7->CR1 &= ~TIM_CR1_CEN; //turn off timers
	TIM1->CR1 &= ~TIM_CR1_CEN;
	TIM2->CR1 |= TIM_CR1_CEN; //enable RX timer

	NVIC_DisableIRQ(TIM7_IRQn);
	NVIC_DisableIRQ(TIM1_CC_IRQn); // TODO: confirm
...
void Afsk_transmitStart(void)
{
	...
	NVIC_EnableIRQ(TIM1_CC_IRQn); // TOOD: confirm
...
void Afsk_transmitStop(void)
{
	...
	TIM6->CCR1 = 44; //set around 50% duty cycle
}
...
void Afsk_init(void)
{
	 ...
	 * TIM7 is the baudrate generator for TX
	 * TIM6 is the PWM generator with no software interrupt
	 * TIM2 is the RX sampling timer with no software interrupt, but it directly calls DMA
	 */

	// These enable clocks for GPIOs. Use GPT to to-discover meaning. Transfered to IOC, best as I can.
	/*
	...
	*/
	...
	// TODO: ADC calibration maybe

	//prepare DMA
	DMA1_Channel1->CCR |= DMA_CCR_MSIZE_0; //16 bit memory region
	DMA1_Channel1->CCR &= ~DMA_CCR_MSIZE_1;
    DMA1_Channel1->CCR |= DMA_CCR_PSIZE_0;
	DMA1_Channel1->CCR &= ~DMA_CCR_PSIZE_1;

	DMA1_Channel1->CCR |= DMA_CCR_MINC | DMA_CCR_CIRC| DMA_CCR_TCIE; //circular mode, memory increment and interrupt
    DMA1_Channel1->CNDTR = 4; //4 samples
	DMA1_Channel1->CPAR = (uint32_t)&(ADC1->DR); //ADC data register address
	DMA1_Channel1->CMAR = (uint32_t)modState.samples_oversampling; //sample buffer address
	...
	if(afskCfg.flatAudioIn) //when used with flat audio input, use de-emphasis and flat modems
	{
		demod1.emphasis = EMPHASIS_NONE;
		demod2.emphasis = DEEMPHASIS;
	}
	else //when used with normal (filtered) audio input, use flat and pre-emphasis modems
	{
		demod1.emphasis = EMPHASIS_NONE;
	...
	if(afskCfg.usePWM)
	{
		// RCC->APB1ENR |= RCC_APB1ENR_TIM6EN; // configure timer
		RCC->AHB1ENR |= RCC_APB1ENR1_TIM6EN; // configure timer
		...
		*/
		...
		TIM6->PSC = 7; //72MHz/8=9MHz
		TIM6->ARR = 90; //9MHz/90=100kHz

		TIM6->CCMR1 |= TIM_CCMR1_OC1M_1 | TIM_CCMR1_OC1M_2;
		TIM6->CCER |= TIM_CCER_CC1E;
		TIM6->CCR1 = 44; //initial duty cycle
