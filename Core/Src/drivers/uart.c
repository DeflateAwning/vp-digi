...
{
	if(port->port->ISR & USART_ISR_RXNE) //byte received
	{
		port->port->ISR &= ~USART_ISR_RXNE;
		port->bufrx[port->bufrxidx] = port->port->RDR; //store it
		port->bufrxidx++;
	...
	if(port->port->ISR & USART_ISR_IDLE) //line is idle, end of data reception
	{
		port->port->RDR; //reset idle flag by dummy read
		if(port->bufrxidx == 0)
			return; //no data, stop
	...
	if(port->port->ISR & USART_ISR_TXE) //TX buffer empty
	{
		if(port->buftxrd != port->buftxwr) //if there is anything to transmit
		{
			port->port->RDR = port->buftx[port->buftxrd++]; //push it to the refister
