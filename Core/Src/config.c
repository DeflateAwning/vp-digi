...
{
	erase_flash_memory(MEM_CONFIG, 2048);
}

void erase_flash_memory(uint32_t start_address, uint32_t size_in_bytes) {
	// source: ChatGPT

    HAL_FLASH_Unlock(); // Unlock the Flash memory for write/erase operations

    // Calculate the page number corresponding to the start address
    uint32_t start_page = (start_address - FLASH_BASE) / FLASH_PAGE_SIZE;

    // Calculate the number of pages to erase
    uint32_t num_pages = (size_in_bytes + FLASH_PAGE_SIZE - 1) / FLASH_PAGE_SIZE;

    // Erase the pages
    FLASH_EraseInitTypeDef erase_init;
    erase_init.TypeErase = FLASH_TYPEERASE_PAGES;
    erase_init.Page = start_page * FLASH_PAGE_SIZE + FLASH_BASE;
